...
ковариация - мера совместной изменчивости двух случайных 

чем больше значение тем выше взаимосвязь

в зависимости от знача зависит тип связи, положительная или отрицательная
...
r<sub>xy</sub> = cov / (σ<sub>x</sub>σ<sub>y</sub>)
![img_6.png](img_6.png)

# Условия применения коэффициента корреляции

Для применения характер взаимосвязи должен быть ленейный и монотонный
...
МНК - метод наименьших квадратов - метода нахождения оптимальных параметров линейно регрессии, таких, что сумма квадратов остатов была минимальной

уравнение регрессии\
![img_7.png](img_7.png)

> Рассчитайте коэффициент b1 для регрессионной прямой, если коэффициент детерминации(R<sup>2</sup>) равен 0,25:

>M<sub>x</sub> = 15 (выборочное среднее)
>D<sub>x</sub> = 25

>M<sub>y</sub> = 10
>D<sub>y</sub> = 36

b<sub>1</sub> = sd<sub>y</sub> / sd<sub>x</sub> * r<sub>xy</sub>

sd = sqrt(D)

r<sub>xy</sub> = sqrt(R<sup>2</sup>)

b<sub>1</sub> = sqrt(36) / sqrt(25) * sqrt(0.25) = 0.6

# Гипотеза о значимости взаимосвязи и коэффициент детерминации

Коэффициент детерминации R<sup>2</sup> - доля дисперсии зависимой переменной (Y) объясняемая регрессионной моделью  
![img_8.png](img_8.png)

# Условия применения линейной регрессии с одним предиктором

1. Линейная зависимость X и Y
...
мультиколлинеарность - очень сильная взаимосвять между какими-то из наших независимых переменных

1. В регрессионном анализе, особенно в множественном, кол-во не всегда означает качество (бездумно добавлять фичи не лучший варинат, особенно без проверки на мультиколлинеарность)
2. Преимущество по сравнению с корреляцие - можно построить матрицу корреляций всех переменных но:
   1. возникает опасность множественных сравнений (потребность компенсирования для избегания роста ложно положительных результатов)
   2. регресионный анализ, а именно его коэффициенты при переменных дают более интересню информацию о взаимосвязи найше зависимой переменной и предиктора, на сколько сильно они взаимосвязаны между собой при условии что мы учли все остальные переменные и они не изменяются(исключается влияние третье переменной) 
